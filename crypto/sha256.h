/*
 * Protheus OS — SHA-256 (FIPS 180-4), header-only
 *
 * Lives exclusively inside crypto_service — never linked into other services.
 *
...
static void sha256_final(sha256_ctx *ctx, uint8_t *digest) {
    uint8_t fc[8];
    for (int i=0; i<8; i++)
        fc[i]=(uint8_t)(ctx->count[(i>=4)?0:1]>>((3-(i&3))*8));
    sha256_update(ctx,(const uint8_t*)"\x80",1);
    while ((ctx->count[0]&504)!=448) sha256_update(ctx,(const uint8_t*)"\0",1);
    sha256_update(ctx,fc,8);
...
/* Undef internal macros to avoid polluting including TUs */
#undef ROTR32
#undef CH
#undef MAJ
#undef EP0
