...
#define PROC_BLOCKED   2   /* blocking on IPC */
#define PROC_EXITED    3

/* ── Capabilities ──────────────────────────────────────────────────────── */
#define CAP_NONE    0x00
#define CAP_CRYPTO  (1 << 0)
#define CAP_KVSTORE (1 << 1)
#define CAP_NETWORK (1 << 2)
#define CAP_TIMER   (1 << 3)
#define CAP_LOG     (1 << 4)
#define CAP_ALL     0xFF
...
#define IPC_STATUS_DENIED   1
#define IPC_STATUS_ERROR    2
#define IPC_STATUS_NOTFOUND 3

/* ── IPC buffer limits ─────────────────────────────────────────────────── */
#define IPC_KEY_MAX   64
#define IPC_VALUE_MAX 512
#define IPC_HASH_LEN  32   /* SHA-256 */
#define IPC_SIG_LEN   64   /* Ed25519 */
...
/* IPC rendezvous buffer — one per process, kernel-mapped, user-readable */
struct ipc_msg {
    uint8_t  type;
    uint8_t  status;
    uint16_t payload_len;
    uint8_t  payload[IPC_VALUE_MAX];
...
struct process {
    ...
    uint32_t       *page_table;
    uint8_t         stack[8192];
    struct ipc_msg *ipc_buf;   /* kernel-allocated shared page */
};
...
struct virtq_avail {
    uint16_t flags; uint16_t index; uint16_t ring[VIRTQ_ENTRY_NUM];
} __attribute__((packed));
...
struct virtio_blk_req {
    ...
    uint8_t  data[512]; uint8_t status;
} __attribute__((packed));
...
struct tar_header {
    ...
    char uname[32], gname[32], devmajor[8], devminor[8], prefix[155];
    char padding[12];
    char data[];
} __attribute__((packed));
...
/* ── Kernel API (used by services via syscalls, not direct calls) ───────── */
void yield(void);
